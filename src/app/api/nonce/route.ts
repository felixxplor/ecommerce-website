...
export async function POST(request: Request) {
  try {
    const { sessionToken, authToken, clientSessionId, timeout } =
    ...
    if (!clientSessionId || !timeout) {
      return NextResponse.json(
        { errors: { message: 'Client Session ID and expiration must be provided.' } },
        { status: 500 }
      )
    }
    ...
    const results = await client.request<UpdateSessionMutation>(UpdateSessionDocument, { input })
